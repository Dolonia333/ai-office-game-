...
| **Nearby NPCs with tags** | `- Nearby: Josh (walking, walking with you), Bob (idle, last spoke 22m ago)` | Per-person richness — state, convoy detection, peer-contact recency |
...
| **Mood / sentiment** | "Alex sounded frustrated in his last message" | Run a sentiment classifier on the last message — could reuse LLM but cheap-pass on keyword regex; ~45 min |
...
| **Conversation thread continuity** | "This is the 3rd time Sarah has asked about the mockups today" | Hash conversation topic + count occurrences; ~30 min |
