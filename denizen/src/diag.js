...
(function () {
  ...
  function hasRecentBrainActivity(snap) {
    const st = snap?.stats || {};
    const now = Date.now();
    const lastBrainOk = Math.max(
      Number(st.lastProviderOkAt || 0),
      Number(st.lastNpcConvAt || 0),
      Number(st.lastPlayerChatAt || 0),
    );
    return Boolean(lastBrainOk && (now - lastBrainOk) < 30000);
  }

  function healthUrls() {
    const urls = [HEALTH_URL];
    try {
      ...
      if (isFile) {
        urls.push('http://127.0.0.1:8080/api/health', 'http://localhost:8080/api/health');
      } else {
        if (port !== '8080') {
          urls.push(`${window.location.protocol}//${host}:8080/api/health`);
        }
        if (host !== '127.0.0.1') {
          urls.push(`${window.location.protocol}//127.0.0.1:8080/api/health`);
        }
        if (host !== 'localhost') {
          urls.push(`${window.location.protocol}//localhost:8080/api/health`);
        }
      }
    } catch (_) {}
  ...
  function renderChip(snap) {
    ...
    const haveAnyBrain = lm.reachable || remoteProviders.length > 0 || hasRecentBrainActivity(snap);
    ...
    try {
      for (const u of urls) {
        try {
          ...
          if (u !== HEALTH_URL && lastSnapshot && typeof lastSnapshot === 'object') {
            lastSnapshot._healthUrl = u;
          }
          lastErr = null;
          break;
        } catch (err) {
