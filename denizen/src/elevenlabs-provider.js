...
(function () {
  ...
  function showAudioUnlockHint() {
    const id = 'denizen-audio-unlock-hint';
    ...
    el.textContent = 'Audio blocked by browser policy. Click anywhere in the page once to unlock voice.';
    document.body.appendChild(el);
    const remove = () => {
      try { el.remove(); } catch (_) {}
      window.removeEventListener('pointerdown', remove);
      window.removeEventListener('keydown', remove);
    };
    window.addEventListener('pointerdown', remove, { once: true });
